...
- Result: completed
- Changes: The `PcaModel`-based `flatten_pca`/`fit_flattened_pca`/`append_pca_scores` migration was already implemented, but `reshape_pca_components` selected its returned long-form `pl.DataFrame` columns in the wrong order (`Time, Step, Sequence, wavelength, component, coefficient`). Corrected the final `.select(...)` in `src/flat_pca/feature_engineering/flatten_pca/component_reshape.py` to the required `component, wavelength, Step, Sequence, Time, coefficient` order.
...
## 2026-09-19 02:38 - TASK-019

- Result: completed
- Changes: Added `target_steps` and `edge_trim` to `preprocess_and_flatten` and `flatten_pca` (`paths` branch). Added a new `step_time.py` module with `filter_target_steps` (Step行フィルタ), `add_step_time_columns` (inserts `StepTime`/`ReverseStepTime` after `Time`, restarting at each Time-ordered Step/Sequence change and reversing per segment), and `apply_edge_trim` (nulls non-meta columns where `StepTime < edge_trim[0]` or `ReverseStepTime < edge_trim[1]`). Wired these into `preprocess_and_flatten` immediately after input validation and before smoothing/normalization/downsampling/flatten. Updated `flatten.py` to sort and name flattened features by `StepTime` instead of `Time`. Updated `schema.py`'s `wavelength_columns` to also exclude `StepTime`/`ReverseStepTime`, and `downsampling.py`'s `apply_w_downsampling` to preserve whatever non-spectral columns are present (previously hardcoded to `METADATA_COLUMNS`, which would have dropped the new columns).
- Tests: Added `tests/feature_engineering/test_flatten_pca_step_time.py` covering target_steps row filtering/column-count reduction (and `None` no-op), StepTime/ReverseStepTime segment-boundary and revisit behavior, StepTime-based (not Time-based) flatten feature naming, edge_trim nulling of non-meta columns with meta preserved (and `None` no-op), and an end-to-end run combining target_steps, edge_trim, and downsampling — all derived in-memory from `tests/fixtures/real_subset` (whose Step column is otherwise constant) per the project's fixture policy. Updated `test_flatten_pca.py`, `test_flatten_pca_flatten.py`, and `test_flatten_pca_downsampling.py` to insert the now-mandatory `StepTime`/`ReverseStepTime` columns via `add_step_time_columns` wherever they build frames or expected results outside the public API, and to add the new keyword-only parameters to the `flatten_pca` signature-stability assertion. `uv run -m pytest tests/feature_engineering/test_flatten_pca_step_time.py` (6 passed); `uv run -m pytest tests/feature_engineering/test_flatten_pca.py` (25 passed); `uv run -m pytest` (123 passed); `uv run -m ruff check .` (passed); `git diff --check` (passed).
- Requirements: Verified target_steps filtering and its `None` no-op, StepTime/ReverseStepTime generation (including a non-contiguous Step revisit that restarts a new segment), edge_trim's independent meta-preserving overwrite and its `None` no-op, StepTime-based flatten feature naming, and the required Step-filter/StepTime/edge_trim → smoothing/normalization/downsampling/flatten processing order — all via the new tests plus the updated existing suite.
- Notes: `edge_trim`'s type hint uses the literal `list[float, float]` from `SPEC.md`'s public signature (not standard-conforming, but preserved as specified since `from __future__ import annotations` keeps it unevaluated). No fixture Parquet files were modified.
